...
if(in_array("search-no-results",get_body_class())){ ?>
   <div class="breadcrumb" class="col-sm-12">
     <a href="<?php esc_url( home_url( '/' )); ?>"><?php echo esc_html__('Home', 'mafoil'); ?></a>
   <span class="delimiter"></span>
   <span class="current"><?php echo esc_html__('Search results for : ', 'mafoil') .'"' . esc_html(get_search_query()) . '"'; ?></span> </div>
<?php
    }else{
        ...
        echo '<div id="breadcrumb" class="breadcrumb">';
			...
			echo '<a href="' . esc_url( $homeLink ). '">' . esc_html__('Home', 'mafoil') . '</a> ' . wp_kses($delimiter,'social') . ' ';
			if ( is_category() ) {
				...
				echo wp_kses($before,'social') . esc_html__('Archive by date ','mafoil') .'"' . esc_html(get_the_time('d')) .'"' . wp_kses($after,'social');
				echo '</div>';
			} elseif ( is_month() ) {
				echo '<a href="' . esc_url(get_year_link(get_the_time('Y'))) . '">' . esc_html(get_the_time('Y')) . '</a> ' . wp_kses($delimiter,'social') . ' ';
				echo wp_kses($before,'social') . esc_html__('Archive by month ','mafoil') .'"' . esc_html(get_the_time('F')) .'"' . wp_kses($after,'social');
				echo '</div>';
			} elseif ( is_year() ) {
				echo wp_kses($before,'social') . esc_html__('Archive by year ', 'mafoil') .'"' . esc_html(get_the_time('Y')) . '"' . wp_kses($after,'social');
				echo '</div>';
				...
				echo '</div>';
				 ...
				 echo wp_kses($before,'social') . esc_html__('Search results for : ','mafoil') .'"' . esc_html(get_search_query()) . '"' . wp_kses($after,'social');
				echo '</div>';
			} elseif ( is_tag() ) {
				echo wp_kses($before,'social') . esc_html__('Archive by tag ','mafoil') .'"' . esc_html(single_tag_title('', false)) . '"' . wp_kses($after,'social');
				echo '</div>';
			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata($author);
				echo wp_kses($before,'social') . esc_html__(' Articles posted by ','mafoil') .'"' . esc_html($userdata->display_name) . '"' . wp_kses($after,'social');
				echo '</div>';
			} elseif ( is_404() ) {
				echo wp_kses($before,'social') . esc_html__('You got it ','mafoil') .'"' . esc_html__(' Error 404 not Found ','mafoil') . '"&nbsp;' . wp_kses($after,'social');
				echo '</div>';
			}else{
				echo wp_kses($before,'social') . esc_html__('Blog','mafoil') . wp_kses($after,'social');
				echo '</div>';
			}	
