...
class gz_rewrite_ml {
  ...
  public static function do_parse_request($cur) {
    ...
    public static function get_default_lang(){
        return apply_filters('gz_rewrite_ml_default_lang', 'th');
    }

  public static function add_directory_rewrite() {
    //if(isset($_GET['d'])) die(current_action());
    global $wp_rewrite;
    $supported_languages = self::get_supported_langs();
    //if(isset($_GET['d'])) {ob_clean(); die("<pre>".print_r(compact('supported_languages','wp_rewrite'),true)); }
    add_rewrite_tag('%gz_lang%', '('.implode('|', $supported_languages).')');
    add_rewrite_rule('^/('.implode('|', $supported_languages).')/(.*)$','index.php?gz_lang=$matches[1]','bottom');
  }
  ...
  public static function final_rules_correction($rules) {
    ...
    foreach ($rules as $k => $v) {
      if (preg_match($find_find, $k)) {
        ...
        $nv = $index.'?gz_lang='.$wp_rewrite->preg_index(1).(!empty($pv) ? '&'.$pv : '');
        $new_rules[$nk] = $nv;
      } else {
        $new_rules[$k] = $v;
      }
  ...
  public static function change_permalink_structure($struct) {
    ...
    $struct = preg_replace('#^%gz_lang%/?#', '', $struct);
    return '/%gz_lang%/'.$struct;
  }
  ...
  protected static function correct_extras() {
    ...
    public static function get_default_post_lang($post) {
        return ( $lang = get_query_var('gz_lang') ) ? $lang : self::get_default_lang();
    }
  ...
  public static function rewrite_lang_in_permalink($permalink, $post=0, $leavename=false) {
    ...
    public static function add_lang_query_var($vars) {
        ...
        //$vars[] = 'gz_lang';
        //if(isset($_GET['d'])) {ob_clean(); die("<pre>".print_r(compact('vars'),true)); }
        return array_unique($vars);
    }

    public static function default_language($vars) {
        if (array_diff( array_keys($vars), array('preview', 'page', 'paged', 'cpage') ))
            $vars['gz_lang'] = !empty($vars['gz_lang']) ? $vars['gz_lang'] : self::get_default_lang();
        //$vars['gz_lang'] = !empty($vars['gz_lang']) ? $vars['gz_lang'] : self::get_default_lang();
        return $vars;
    }
...
if (defined('ABSPATH') && function_exists('add_action')) {
    gz_rewrite_ml::pre_init();
    /*
    if(isset($_GET['drwml'])) {global $wp_query; ob_clean(); die("<pre>".print_r([
