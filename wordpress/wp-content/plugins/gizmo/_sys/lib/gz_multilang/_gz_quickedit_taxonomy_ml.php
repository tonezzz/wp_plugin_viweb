...
class gz_quickedit_taxonomy_ml extends gz_tpl{
	public function __construct($text_domain=false){ //if(isset($_GET['d'])) die(__FILE__);
		$config = [
			'actions' => [
				['prm'=>['quick_edit_custom_box',[$this,'quick_edit_custom_box_ml'], 10, 2]],
            ]
        ];
    }
/**
	...
	function nav_cmb2_admin_init(){
		...
		add_action('wp_nav_menu_item_custom_fields',[$this,'quickedit_custom_fields'],10,4);
		add_action('wp_update_nav_menu_item',[$this,'wp_update_nav_menu_item'],10,3);
	}
	
	function quick_edit_custom_box_ml($column_name, $post_type){ if(!isset($_GET['d'])) return;
		$html = "";
		$html.= "<div>";
	...
	function quickedit_custom_fields($item_id, $menu_item, $depth, $args ) {
